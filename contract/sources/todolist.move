module todolist_addr::todolist {
    ...
    // Todo list resource stored per account
    ...
    // Task struct stored inside the TodoList table
    ...
    // Create an empty list (resource) under the signer account
    ...
    // Create a task (adds to table + emits event)
    public entry fun create_task(account: &signer, content: String) acquires TodoList {
        let signer_address = signer::address_of(account);
        // must have created a list first
    ...
    public entry fun account_can_not_update_task(admin: signer) acquires TodoList {
        account::create_account_for_test(signer::address_of(&admin));
        // account without list cannot complete a task
