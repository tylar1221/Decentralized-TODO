...
async function publish() {
  ...
  try {
    console.log("Publishing package to new object...");
    const response = await move.createObjectAndPublishPackage({
      ...
      extraArguments: [
        ...
        "--max-gas=30000", // Add higher gas limit
        "--gas-unit-price=100", // Set gas unit price
        "--assume-yes", // Auto-confirm prompts
      ],
    });

    console.log(`Package published successfully to object: ${response.objectAddress}`);
    ...
    console.log(`Module address saved to .env: ${response.objectAddress}`);
  } catch (error) {
